...
export function Filter({ setFilters, filters }: FilterProps) {
    ...
    const [time, setTime] = useState<typeof Times>();
    ...
    function applyFilter() {
        ...
        if (courseRef.current!.value) {
            ...
            if (!courseFilters.includes(courseRef.current!.value.toUpperCase()))
                courseFilters.push(courseRef.current!.value.toUpperCase());
        }
        if (facultyRef.current!.value) {
            ...
            if (
                !facultyFilters.includes(
                    facultyRef.current!.value.toUpperCase(),
                )
            )
                facultyFilters.push(facultyRef.current!.value.toUpperCase());
        }
        if (day) {
            if (!dayFilters) {
    ...
    return (
        <div className="mt-0">
            <Popover>
                <PopoverTrigger>
                    <Button
                        className="mt-0 w-full bg-card p-3"
                        variant="outline">
                        <ListFilter />
                    </Button>
                </PopoverTrigger>
                <PopoverContent className="grid gap-4">
                    ...
                    <div className="grid items-center gap-4">
                        ...
                        <Select
                            onValueChange={(value: Times) => setTime(value)}>
                            <SelectTrigger>
                        ...
                        <Button
                            className="col-span-2 mt-3 bg-foreground text-background hover:text-foreground"
                            onClick={applyFilter}>
                            Apply Filters
                        </Button>
                    </div>
