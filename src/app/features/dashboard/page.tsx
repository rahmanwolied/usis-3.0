...
type DaySchedule = {
  [key in 'Sunday' | 'Monday' | 'Tuesday' | 'Wednesday' | 'Thursday' | 'Friday' | 'Saturday']: string;
};
...
export default function Component() {
  ...
  const days: (keyof DaySchedule)[] = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
      ...
      course.startTimes.forEach((startTime, index) => {
        if (timeSlot === `${startTime} - ${course.endTimes[index]}`) {
          ...
          course.days.forEach(day => {
            if (row[day as keyof DaySchedule] === "") { // Only fill if the cell is empty
              row[day as keyof DaySchedule] = `${course.code} (${course.facultyInitial}) ${course.roomNumber}`;
            }
          });
        }
      });
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {completedCredits} out of {totalCredits} credits completed
          </p>
        </CardContent>
      </Card>
            ...
            {currentCourses.map((course, index) => (
              <Card key={index}>
                <CardContent className="flex justify-between items-center p-4">
                  ...
                  <span className="text-muted-foreground">{course.facultyInitial}</span>
                </CardContent>
              </Card>
            ))}
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {classRoutine.map((row, index) => (
                  <TableRow key={index}>
                    <TableCell className="font-medium text-xs">{row.time}</TableCell>
                    {days.map((day) => (
                      <TableCell key={day} className="p-1">
                        {row[day] && (
                          <div className="bg-secondary text-secondary-foreground rounded p-1 text-xs">
                            {row[day].split(' ').map((item, i) => (
                              <div key={i} className={i === 1 ? 'text-primary' : ''}>{item}</div>
                            ))}
                          </div>
                        )}
                      </TableCell>
